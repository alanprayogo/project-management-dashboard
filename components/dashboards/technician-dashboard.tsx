...
import { Wrench, Clock, CheckCircle, Calendar, MapPin, User } from "lucide-react"
import { useProject } from "@/components/project-context"

export function TechnicianDashboard() {
  const { projects, updateProject } = useProject()
  const [attendanceMarked, setAttendanceMarked] = useState<string[]>([])
  const [progressUpdate, setProgressUpdate] = useState<{ [key: string]: { progress: number; notes: string } }>({})

  // For demo purposes, we'll show projects assigned to "John Doe"
  const myProjects = projects.filter(
    (p) => p.assignedTechnicians?.includes("John Doe") && (p.status === "scheduled" || p.status === "in-progress"),
  )
  ...
  const updateProgressData = (projectId: string, field: "progress" | "notes", value: number | string) => {
    setProgressUpdate((prev) => ({
      ...prev,
      [projectId]: {
        ...prev[projectId],
    ...
    <div className="p-4 md:p-6 space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{myProjects.filter((p) => p.status === "in-progress").length}</div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Projects assigned to you and their current status</CardDescription>
        </CardHeader>
        <CardContent>
          {myProjects.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">No projects assigned to you</p>
          ) : (
                ...
                <div key={project.id} className="border rounded-lg p-4 space-y-4">
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                    ...
                    <div className="flex flex-col gap-2 min-w-fit">
                      {project.status === "scheduled" && !attendanceMarked.includes(project.id) && (
                        <Button
                  ...
                  {project.status === "in-progress" && (
                    <div className="space-y-4 pt-4 border-t">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2">
                          <Label htmlFor={`progress-${project.id}`}>Update Progress (%)</Label>
                          <Input
                            ...
                            onChange={(e) =>
                              updateProgressData(project.id, "progress", Number.parseInt(e.target.value) || 0)
                            }
                            placeholder="Enter progress percentage"
                          />
                        </div>
                        <div className="space-y-2">
                          <Label htmlFor={`notes-${project.id}`}>Progress Notes</Label>
                          <Textarea
                            id={`notes-${project.id}`}
                            value={progressUpdate[project.id]?.notes || ""}
                            onChange={(e) => updateProgressData(project.id, "notes", e.target.value)}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      <div className="space-y-1">
                        ...
                        <p className="text-muted-foreground">
                          {project.assignedTechnicians?.join(", ") || "Not assigned"}
                        </p>
                      </div>
                      <div className="space-y-1">
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-3">
              {myProjects.filter((p) => p.scheduledDate === new Date().toISOString().split("T")[0]).length === 0 ? (
                <p className="text-center text-muted-foreground py-4">No projects scheduled for today</p>
              ) : (
                myProjects
                  .filter((p) => p.scheduledDate === new Date().toISOString().split("T")[0])
                  .map((project) => (
